...
小鼠證據將 MGI 與 IMPC 合併為單一來源：IMPC 資料會透過共用的
Mammalian Phenotype ontology 併入 MGI，因此不將 IMPC 視為獨立來源另行加分
（否則會重複計算同一份小鼠證據）。MGI 與 IMPC 的 MP 詞彙取聯集後只計一次；
若某基因的小鼠證據僅來自 IMPC，仍視為具有小鼠表型。
```
